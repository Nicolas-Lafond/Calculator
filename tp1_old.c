 ...
 * numbers with a number of digit greater than MAX_INT are not supported
 * input are taken on stdin
 */
...
typedef struct cell {int chiffre; struct cell *suivant; struct cell *precedent;} cell;
typedef struct num {int positif; struct cell *chiffre;} num;
typedef struct maillon {struct num *nombre; struct maillon *suivant;} maillon;
typedef struct pile { struct maillon *dessus;} pile;
...
void push(pile *p, num *a) {
    maillon *m = p->dessus;
    p->dessus = malloc(sizeof(maillon));
    p->dessus->nombre = a;
    p->dessus->suivant = m;
}

num* pop(pile *p) {
    maillon *top = p->dessus;
    num *r = top->nombre;
    p->dessus = top->suivant;
...
int main() {
     ...
     /*recuperation de la chaine de caractères*/
    int i=1;// taille de la chaine entrée
    ...
    while (i !=0){
        ...
        while (s != '\n'){
			...
			if (i == taille) {
                taille *= 2;
                chaine=  realloc(chaine,taille*sizeof(char));
            }
            s= getchar();


        }
        traitement_chaine(chaine,i);       
		valid:s='N';// affin que s!=\n
        free(chaine);
